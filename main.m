...
if strcmp(database,'Indian')
    ...
    ratio_train = 0.05;      AF = 'ReLU';      ratio_aug = 0.005;      pixel = 100;
    data3D = indian_pines_corrected;      label_gt = indian_pines_gt;
    h_score = 2.5;      l_score = 0.5;%2.5
    
elseif strcmp(database,'KSC')
    load('KSC.mat');load('KSC_gt.mat');
    load('RK.mat');load('BK_A.mat');load('BK_B.mat');load('IWK_A.mat');load('IWK_B.mat');
    ratio_train = 0.05;      AF = 'ReLU';       ratio_aug = 0.005;       pixel = 91;
...
elseif strcmp(database,'PaviaU')
    ...
    ratio_train = 0.05;      AF = 'tanh';       ratio_aug = 0.005;       pixel = 23;
    data3D = paviaU;       label_gt = paviaU_gt;
    h_score = 3;      l_score = 0.5;%3
...
elseif strcmp(database,'Salinas')
    ...
    ratio_train = 0.05;      AF = 'sig';       ratio_aug = 0.005;       pixel = 100;%100
    data3D = salinas_corrected;      label_gt = salinas_gt;
    h_score = 3.5;      l_score = 0.5;%3.5
end
...
[ndata, ~] = size(data); 
num_train = round(ratio_train*ndata);
% R = randperm(ndata);
...
% [IW1,B1,LW1,AF1,TYPE1] = elmtrain(DataTrn',LabTrn',2000,AF,1); 
...
% [IW2,B2,LW2,AF2,TYPE2] = elmtrain(DataTrn',LabTrn',2000,AF,1);
...
%% Score processing
ScoreTst_A = sort(max(Score_A),'descend');
ScoreTst_B = sort(max(Score_B),'descend');
C = [];D = [];
DataTrn_A = DataTrn;
DataTrn_B = DataTrn;
LabTrn_A = LabTrn;
...
iter = 9;
num_aug = round(ratio_aug*ndata);
for x = 1:iter
    ScoreTst_A = sort(ScoreTst_A,'descend');
    ScoreTst_B = sort(ScoreTst_B,'descend');
    ...
    t3 = 0; t4 = 0;
    for i = 1:Len_GA
        if(GuideScore_A(i) >= GuideScore_A(num_aug))
            L3(i) = 1;
            t3 = t3+1;
        end
    end
    for i = 1:Len_GB
        if(GuideScore_B(i) >= GuideScore_B(num_aug))
            L4(i) = 1;
            t4 = t4+1;
        end
    end
    ...
    GuideData_A = DataRTst_A(indexCor3,:);
    GuideData_B = DataRTst_B(indexCor4,:);
    ...
    %training set
    DataRTrn_A = [DataTrn_A;GuideData_B];
    LabRTrn_A = [LabTrn_A;GuideLab_B];
    DataRTrn_B = [DataTrn_B;GuideData_A];
    LabRTrn_B = [LabTrn_B;GuideLab_A];
...
Result=[Result_OA;Result_AA;Result_Kappa];
% xlswrite('result_training.xlsx',Result,'sheet2','D11');
